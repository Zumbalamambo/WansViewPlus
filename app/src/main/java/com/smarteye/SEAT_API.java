...
public class SEAT_API {
    ...
    static {
        try {
            ...
            WLog.d("SEAT_API", "loadLibrary(SEAT_API), api_ver==" + ms_verAPI);
        } catch (UnsatisfiedLinkError ule) {
            WLog.d("SEAT_API", "loadLibrary(SEAT_API)," + ule.getMessage());
        }
    }
    ...
    //--interface----------------------------------------------------------
    private static IMsgConfig g_iconfigok = null;

    public void setInterfaceConfigOK(IMsgConfig iconfigok) {
        g_iconfigok = iconfigok;
    }

    public interface IMsgConfig {
        void configOK(String strMsg);
        void onBegin();
        void onEnd();
    }
    ...
    void POnBegin(int pUserData, int nATType, float fSoundTime) {
        WLog.d("SEAT_API","POnBegin] nATType=" + nATType);
        if (g_iconfigok != null) g_iconfigok.onBegin();
    }
    ...
    void POnEnd(int pUserData, int nATType, float fSoundTime, int nResult, byte[] pData, int nDataLen) {
        ...
        WLog.d("SEAT_API", str);
        if (g_iconfigok != null) g_iconfigok.onEnd();
    }
    ...
    void PConfigOK(int pUserData, byte[] bytConfigOK, int nBytSize) {
        ...
        if (g_iconfigok != null) g_iconfigok.configOK(stConfig.getWiFi_ssid());
    }
    ...
    //如果没有设备ID，pDID=null, nDIDLen=0
    public native int SEAT_WriteSSIDWiFi2(int pHandle, int chAppType, byte[] pSSID, int nSSIDLen, byte[] pPwd, int nPwdLen, int eWifiMode, byte[] pMACAddrWithColon, byte[] pDID, int nDIDLen);
