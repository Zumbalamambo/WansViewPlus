...
//	}
	...
	public static boolean isFirstCharacter(String key, String str) {
		try {
	...
	public static boolean isStartPinYin(String key, String str) {
	...
	public static boolean isContainsPinYin(String key, String str) {
		...
		List<List<String>> allChinese = new ArrayList<List<String>>();
		for (char c : str.toCharArray()) {
			try {
				String[] pinyinArray = PinyinHelper.toHanyuPinyinStringArray(c,
						outputFormat);
				if (pinyinArray != null) {
					List<String> list = Arrays.asList(pinyinArray);
					allChinese.add(list);
				}
			} catch (Exception e) {
				e.printStackTrace();
		...
		getAllChinese(allChinese, allList, 0, "");
		for (String chinese : allList) {
	...
	private static void getAllChinese(List<List<String>> dimvalue,
                                      List<String> result, int layer, String curstring) {
		// 大于一个集合时：
		if (layer < dimvalue.size() - 1) {
			// 大于一个集合时，第一个集合为空
			if (dimvalue.get(layer).size() == 0)
				getAllChinese(dimvalue, result, layer + 1, curstring);
			else {
				for (int i = 0; i < dimvalue.get(layer).size(); i++) {
					StringBuilder s1 = new StringBuilder();
					s1.append(curstring);
					s1.append(dimvalue.get(layer).get(i));
					getAllChinese(dimvalue, result, layer + 1, s1.toString());
				}
			}
		}
		// 只有一个集合时：
