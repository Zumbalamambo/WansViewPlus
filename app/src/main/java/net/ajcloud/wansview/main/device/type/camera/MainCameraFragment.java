...
import net.ajcloud.wansview.entity.camera.CameraStatus;
...
import net.ajcloud.wansview.support.customview.camera.DragLayout;
import net.ajcloud.wansview.support.customview.camera.NavigationPopupWindow;
import net.ajcloud.wansview.support.customview.camera.PTZView;
import net.ajcloud.wansview.support.customview.camera.ScaleFrameLayout;
import net.ajcloud.wansview.support.customview.camera.VideoPlayArea4To3;
import net.ajcloud.wansview.support.customview.camera.VideoQuality;
import net.ajcloud.wansview.support.utils.AudioUtils;
import net.ajcloud.wansview.support.utils.CameraUtil;
import net.ajcloud.wansview.support.utils.ConnectivityUtil;
import net.ajcloud.wansview.support.utils.FileUtil;
import net.ajcloud.wansview.support.utils.SizeUtil;
...
public class MainCameraFragment extends BaseFragment
    ...
    protected int layoutResID() {
        return net.ajcloud.wansview.R.layout.fragment_ws_main_camera;
    }
    ...
    protected void initView(View rootView) {
        ...
        refreshMenuSetting( net.ajcloud.wansview.R.menu.menu_camer_setting);
        initSurfaceView();
        ShowUpdate(UPDATE_STATUS.EXIT);
    }
    ...
    private void refreshMenuSetting(int menu) {
        ...
        myToolBar.setNavigationIcon(net.ajcloud.wansview.R.drawable.ic_all);
    ...
    public void refreshView() {
        ...
        if (CameraStatus.OFFLINE == online) {
            ...
            if (file.exists()) {
                ...
                realTimeImageImageView.setImageResource(net.ajcloud.wansview.R.mipmap.realtime_picture);
            }
            ...
            offlineImg.setImageResource(net.ajcloud.wansview.R.mipmap.camera_offline);
            offlineImg.setOnClickListener(null);
            offlineTip.setText(getString(net.ajcloud.wansview.R.string.wv_device_not_online));
            offlineHelp.setText(getString(net.ajcloud.wansview.R.string.wv_go_help_center));
...
//            videoHide.setVisibility(View.VISIBLE);
            ...
            offlineImg.setImageResource(net.ajcloud.wansview.R.mipmap.home_sleep);
            offlineImg.setOnClickListener(null);
            offlineTip.setText(getString(net.ajcloud.wansview.R.string.wv_device_sleep));
            offlineHelp.setText(getString(net.ajcloud.wansview.R.string.wv_right_on));
            videoQuality.setVisibility(View.GONE);
            realTimeImageNoWifiTip.setVisibility(View.GONE);
            ShowUpdate(UPDATE_STATUS.EXIT);
            ...
            offlineImg.setImageResource(net.ajcloud.wansview.R.mipmap.play_white);
            ...
            offlineTip.setText(getString(net.ajcloud.wansview.R.string.wv_video_net_week));
            offlineHelp.setText(getString(net.ajcloud.wansview.R.string.wv_goto_net_check));
            realTimeImageNoWifiTip.setVisibility(View.GONE);
            videoQuality.setVisibility(View.GONE);
            ShowUpdate(UPDATE_STATUS.EXIT);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION)) {
                if (ConnectivityUtil.isConnected(getActivity())) {
                    ...
                    realTimeImageNoWifiTip.setVisibility(View.GONE);
                    ToastUtil.show(net.ajcloud.wansview.R.string.wv_error_network);
                    stopVideoPlay();
                }
            }
        }
        ...
        public boolean onTouch(View v, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    recordVoice.setImageResource(net.ajcloud.wansview.R.mipmap.wv_control_phone_click);
                    if (cannotOperateWhenOfflineOrNoplay()) {
                        hHandler.postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                recordVoice.setImageResource(net.ajcloud.wansview.R.mipmap.wv_control_phone);
                            }
                        }, 200);
                        return true;
                    }
...
//                    启动定时器，定时取消方向控件的显示
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case Media.State.NothingSpecial:
                    ...
                    showProgressAndTips(getString(net.ajcloud.wansview.R.string.wv_trying_hard_to_play_and_wait));
                    policeHelper.getUrlAndPlay();
                    preState = Media.State.NothingSpecial;
                    break;
                case Media.State.Opening:
                    Log.d(TAG, "Media.State.Opening");
                    tips.setText(getString(net.ajcloud.wansview.R.string.wv_opening_video_and_wait));
                    if (preState == Media.State.Opening) {
                        if (System.currentTimeMillis() - preStateBeginTime >= 6000) {
                            ...
                            ToastUtil.show(net.ajcloud.wansview.R.string.wv_videoplayer_network_open_tips);
                        }
                    } else {
                ...
                case Media.State.Buffering:
                    ...
                    tips.setText(net.ajcloud.wansview.R.string.wv_playing_video_and_wait);
                    if (preState == Media.State.Buffering) {
                        if (System.currentTimeMillis() - preStateBeginTime >= 4000) {
                            policeHelper.getUrlAndPlay();
                            preStateBeginTime = System.currentTimeMillis();
                            ToastUtil.show(net.ajcloud.wansview.R.string.wv_videoplayer_network_buffer_tips);
                        }
                    } else {
                ...
                case Media.State.Ended:
                    ...
                    showProgressAndTips(getString(net.ajcloud.wansview.R.string.wv_loading_and_wait));
    ...
    private String getLibVlcStateString(int state) {
        switch (state) {
            case Media.State.NothingSpecial:
                return getString(net.ajcloud.wansview.R.string.wv_trying_hard_to_ply);
            case Media.State.Opening:
                return getString(net.ajcloud.wansview.R.string.wv_opening_video);
            case Media.State.Buffering:
                return getString(net.ajcloud.wansview.R.string.wv_buffering_video);
            case Media.State.Playing:
                return getString(net.ajcloud.wansview.R.string.wv_playing_video);
            case Media.State.Paused:
                return getString(net.ajcloud.wansview.R.string.wv_video_paused);
            case Media.State.Stopped:
                return getString(net.ajcloud.wansview.R.string.wv_video_stoped);
            case Media.State.Ended:
                return getString(net.ajcloud.wansview.R.string.wv_video_ended);
            case Media.State.Error:
                return getString(net.ajcloud.wansview.R.string.wv_video_error);
            default:
                return "";
        }
    }
    ...
    private String getPlayType(int playType) {
        switch (playType) {
            case PlayMethod.LAN:
                return getString(net.ajcloud.wansview.R.string.wv_lan);
            case PlayMethod.UPNP:
                return getString(net.ajcloud.wansview.R.string.wv_upnp);
//            case PlayMethod.P2P:
//                return getString(R.string.p2p);
            case PlayMethod.P2PX:
                return getString(net.ajcloud.wansview.R.string.wv_p2px);
            case PlayMethod.TCP_RELAY:
                return getString(net.ajcloud.wansview.R.string.wv_proxy);
            case PlayMethod.RTMP_RELAY:
                return getString(net.ajcloud.wansview.R.string.wv_rtmp_relay);
        }
        return "";
    }
    ...
    public void showCannotPlayDialog() {
        if (!getActivity().isFinishing())
            new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), net.ajcloud.wansview.R.style.AlertDialogCustom))
                    .setCancelable(false)
...
//                mSurface.setVisibility(View.GONE);
    ...
    void initCameraView(View view) {
        ...
        album = (TextView) view.findViewById(net.ajcloud.wansview.R.id.album);
        ...
        if (virtualCamera.mQuality == 2 || virtualCamera.mQuality == 1) {
            videoQuality.setText(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_low);
            fullscreen_quality.setText(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_low);
            virtualCamera.mQuality = 2;
        } else if (virtualCamera.mQuality == 3) {
            if (CameraUtil.isSupport1080P(getCamera().getCapAbility())) {
                videoQuality.setText(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_high);
                fullscreen_quality.setText(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_high);
            } else {
                videoQuality.setText(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_normal);
                fullscreen_quality.setText(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_normal);
            }
        } else if (virtualCamera.mQuality == 4) {
            if (CameraUtil.isSupport1080P(getCamera().getCapAbility())) {
                videoQuality.setText(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_best);
                fullscreen_quality.setText(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_best);
            } else {
                videoQuality.setText(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_high);
                fullscreen_quality.setText(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_high);
            }
        ...
        videoQuality.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (CameraStatus.OFFLINE == online) {
                    ToastUtil.show(net.ajcloud.wansview.R.string.wv_device_offline);
                    return;
                }
...
//        circle_wave_view = (RippleBackground) view.findViewById(R.id.circle_wave_view);
        offlineHelp.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //网络质量差
                if (offlineHelp.getText().toString().equalsIgnoreCase(getString(net.ajcloud.wansview.R.string.wv_goto_net_check))) {
...
//                showRealtimeAnimation();
        ...
        myToolBar.setBackgroundResource(net.ajcloud.wansview.R.drawable.grident_toolbar);
    ...
    private void videoPlay() {
        ...
        if (CameraStatus.FW_UPGRADE == online) {
            ToastUtil.show(net.ajcloud.wansview.R.string.wv_device_updating_not_operate);
            return;
        }
    ...
    private View.OnClickListener addAngleListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (position == -1) {
                ToastUtil.show(net.ajcloud.wansview.R.string.wv_angle_view_full);
                return;
            }
    ...
    public void initRealTimeTalk_Type() {
        ...
        recordVoice.setImageResource(net.ajcloud.wansview.R.drawable.ws_phone_state);
        /*
        //如该是分享摄像机语音功能屏蔽
    ...
    private void showVideoQualityPopupwindow(View anchor) {
        ...
        if (CameraUtil.isSupport1080P(getCamera().getCapAbility())) {
            ...
            view.findViewById(net.ajcloud.wansview.R.id.quality_HD).setVisibility(View.VISIBLE);
            view.findViewById(net.ajcloud.wansview.R.id.quality_SD).setVisibility(View.GONE);
        } else {
            view.findViewById(net.ajcloud.wansview.R.id.quality_FD).setVisibility(View.GONE);
            view.findViewById(net.ajcloud.wansview.R.id.quality_HD).setVisibility(View.VISIBLE);
        }
        ...
        view.findViewById(net.ajcloud.wansview.R.id.quality_FD).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                if (mMediaPlayer.getPlayerState() == Media.State.Opening) {
                    ToastUtil.show(net.ajcloud.wansview.R.string.wv_opening_video_and_wait_to_change);
                    pop_quality.dismiss();
                    return;
                }
                ...
                if (!mMediaPlayer.isPlaying()) {
                    videoQuality.setText(getString(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_best));
                    fullscreen_quality.setText(getString(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_best));
        ...
        view.findViewById(net.ajcloud.wansview.R.id.quality_HD).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (CameraUtil.isSupport1080P(getCamera().getCapAbility())) {
        ...
        view.findViewById(net.ajcloud.wansview.R.id.quality_SD).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (virtualCamera.mQuality == 3) {
                    pop_quality.dismiss();
                ...
                if (!mMediaPlayer.isPlaying()) {
                    videoQuality.setText(getString(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_normal));
                    fullscreen_quality.setText(getString(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_normal));
                    videoScale = 1;
                    virtualCamera.mQuality = 3;
                    Log.d(TAG, "on ChangeQuality to " + virtualCamera.mQuality);
                    //AppApplication.camera2quality.put(virtualCamera.cid, 3);
        ...
        view.findViewById(net.ajcloud.wansview.R.id.quality_Low).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                if (!mMediaPlayer.isPlaying()) {
                    videoQuality.setText(getString(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_low));
                    fullscreen_quality.setText(getString(net.ajcloud.wansview.R.string.wv_videoplayer_menu_quality_low));
                    videoScale = 1;
                    virtualCamera.mQuality = 2;
                    Log.d(TAG, "on ChangeQuality to " + virtualCamera.mQuality);
    ...
    private boolean record() {
        ...
        if (!mMediaPlayer.isRecordable()) {
            ToastUtil.show(net.ajcloud.wansview.R.string.wv_video_cannot_record);
            return true;
        }
        ...
        if (!mMediaPlayer.isRecording()) {
            if (mMediaPlayer.isRecording()) {
                ToastUtil.show(net.ajcloud.wansview.R.string.wv_video_already_recording);
                return true;
            } else {
            ...
            fullscreen_recordvideo.setImageDrawable(getResources().getDrawable(net.ajcloud.wansview.R.mipmap.video_h_click));
        } else {
            stopRecord(true);
            take_video.setSelected(false);
            fullscreen_recordvideo.setImageDrawable(getResources().getDrawable(net.ajcloud.wansview.R.mipmap.video_h));
        }
        ...
        if (active) {
            if (mMediaPlayer != null && mMediaPlayer.isRecording()) {
                ...
                if (Orientation == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT && !TextUtils.isEmpty(videoPicPath)) {
                    ToastUtil.show(net.ajcloud.wansview.R.string.wv_have_save_to_photo);
            ...
            if (mMediaPlayer.isRecording()) {
                ToastUtil.show(net.ajcloud.wansview.R.string.wv_error_network_please_record_again);
            }
        }
    }
    ...
    private boolean snapshot() {
        ...
        if (!directory.exists())
            directory.mkdirs();
        if (!(mMediaPlayer.getPlayerState() == Media.State.Playing)) {
            ToastUtil.show(net.ajcloud.wansview.R.string.wv_video_hasnot_play);
            return true;
        }
        if (mMediaPlayer.takeSnapShot(MainApplication.fileIO.getImageFileDirectory(virtualCamera.cid), 1920, 1080, true)) {
            ...
            ToastUtil.show(net.ajcloud.wansview.R.string.wv_video_snapshot_fail);
            return false;
        }
    }
    ...
    private boolean cannotOperateWhenOfflineOrNoplay(boolean isAllowedContinueClick, int interval) {
        ...
        if (getCamera().getCameraState().getStatus() == CameraStatus.OFFLINE) {
            ...
            ToastUtil.show(net.ajcloud.wansview.R.string.wv_get_verify_quick_error);
            isCanOperate = true;
        ...
        if (mMediaPlayer.takeSnapShot(MainApplication.fileIO.getCacheDir() + virtualCamera.cid + File.separator + index, mVideoWidth, mVideoHeight, true)) {
            if (directory.listFiles().length == 1) {
                try {
                    ...
                    ToastUtil.show(net.ajcloud.wansview.R.string.wv_camera_angle_tip);
...
//            根据视频播放窗和缩放比，计算视频画面最大偏移，缩放时视频画面边界不能偏移到播放窗内
    ...
    public Camera getCamera() {
        if (null == camera) {
            ...
            try {
                JsonObject = new JSONObject(FileUtil.readRawFile(net.ajcloud.wansview.R.raw.experience_camera));
                ...
                JsonObject = new JSONObject(FileUtil.readRawFile(net.ajcloud.wansview.R.raw.experience_camera_state));
                ...
                JsonObject = new JSONObject(FileUtil.readRawFile(net.ajcloud.wansview.R.raw.experience_camera_ability));
                result = JsonObject.getJSONObject("result");
                gson = new Gson();
...
//            tips.setText(getString(R.string.low_quality_try_again));
            ...
            if ((getCamera().getCapAbility().getFeatures().getDiagnose() == 1)) {
                ...
                tips.setText(net.ajcloud.wansview.R.string.wv_current_network_not_well);
                tips.setVisibility(View.VISIBLE);
        ...
        try {
            ...
            if (count == RealTime_picture_wait_times) {
                ...
                if (isFromRealTimeImageButton) {
                    isFromRealTimeImageButton = false;
                    ToastUtil.show(net.ajcloud.wansview.R.string.wv_get_network_fail_please_check);
                }
                realTimeImagePlayRelativeLayout.setVisibility(View.VISIBLE);
                realTimeImageFrameLayout.setVisibility(View.VISIBLE);
                realTimeImagePlay.setVisibility(View.VISIBLE);
    ...
    private void getSnapshot(int wait_time, boolean isShowView) {
        ...
        if (isShowView) {
            ...
            realTimeImage_tip.setText(getString(net.ajcloud.wansview.R.string.wv_get_latest_realtime_image));
            realTimeImageImageView.setVisibility(View.GONE);
            realTimeImagePlayRelativeLayout.setVisibility(View.GONE);
    ...
    void forceOffAudio() {
        if (isRealTimeSpeechOn) {
            ...
            recordVoice.setImageResource(net.ajcloud.wansview.R.drawable.phone_state);
        ...
        public void onClick(View v) {
            ...
            switch (v.getId()) {
                ...
                case net.ajcloud.wansview.R.id.angle:
                    ...
                    break;
                case net.ajcloud.wansview.R.id.dynamic:
    ...
    public void ShowUpdate(UPDATE_STATUS status) {
        ...
        switch (status) {
            case SUCCESS:
                ...
                upgrade_info.setText(net.ajcloud.wansview.R.string.wv_camera_upgrade_success);
            ...
            case TIMEOUT:
                ...
                updateProgressLayout.setVisibility(View.GONE);
                upgrade_info.setVisibility(View.VISIBLE);
                upgrade_info.setText(net.ajcloud.wansview.R.string.wv_camera_upgrade_failture);
                HideUpdate_Handler.postDelayed(HideUpdateRunable, 8000);
                break;
            case PROCESSING:
                Update_LinearLayout.setVisibility(View.VISIBLE);
