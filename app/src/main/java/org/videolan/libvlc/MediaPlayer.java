...
public class MediaPlayer extends VLCObject<MediaPlayer.Event> {
    private static final String TAG = "VLC/MediaPlayer";
    ...
    private String mAudioOutput = "opensles";
    ...
    public void setCrop(int scale, int height, int width, Point point, int dw) {
//        if (scale == 4)//变焦实际为3倍
//            scale = 3;
//        int h = (int) (height / Math.sqrt(scale));
//        int w = (int) (width / Math.sqrt(scale));
//        int top = (height - h) / 2;
//        int left = (width - w) / 2;

        float viewScale = (float)width / dw;
        int x = (int)(point.x * viewScale);
        int y = (int)(point.y * viewScale);
        ...
        String geometry = (w + left) + "x" + (h + top) + "+" + left + "+" + top;
//        String geometry = "1216x684+256+144";
//        if (BuildConfig.DEBUG)
        Log.d(TAG, geometry);
        setCrop(geometry);
    }

    public boolean takeSnapShot(String file, int width, int height, boolean isHide) {
        return takeSnapShot(0, file, width, height, isHide);
    }
    ...
    /* 变焦 */
    private native boolean setCrop(String cropGeometry);
    /* 截屏 */
    private native boolean takeSnapShot(int num, String file, int width, int height, boolean hide);
    /* 录制视频 */
    private native boolean videoIsRecording();
    /* 是否可录制 */
    private native boolean videoIsRecordable();
    /* 结束录制 */
    private native boolean videoRecordStop();
    /* 开始录制视频 */
    private native boolean videoRecordStart(String path);
    /* 获取列表播放状态 */
    public native Map<String, Object> getStats();
    /* 设置vlc语音状态 */
    public native void SetRealTimeTalkFlag(int value);

    public native void CldAlgInit(int sample);

    public native int GetAudioSampleRate();

    public native void GetEfData(int length, byte[] outBuffer);
    /*是否开启播放音频的缓冲队列*/
    public native void OpRealTime(int value, int sample);
    public native void audioEchoCancel(byte[] MicBuffer,int MicBufferLength,byte[] outBuffer);
    public native int RealTimeTalk(byte[] buffer,int length);

    /*获取当前视频分辨率*/
    public native int GetHeight();
    public native int GetWidth();

    /*通知VLC当前视频清晰度，用来做屏幕下方偏移*/
    public native int SetVisiableQuality(int quality);

    /*8k/16K自适应相关接口*/
    public native ReverseAudioInfo ReverseAudioGetDesc();
    //public native int ReverseAudioSetParameter(ReverseAudioInfo info);
    public native int ReverseAudioSetParameter(int eType, int iSampleRate, int iChannels, int eTransport);
    public native int ReverseAudioPlay();
    public native int ReverseAudioStop();

    public native int RealTimeTalkRtp(byte[] buffer, int length, int timestamp);
}
